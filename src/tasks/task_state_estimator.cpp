...
task_state_estimator::state_to_obs_jacob(const state_vec_t& state) const noexcept
{
    ...
    // d(a_exp)/d(qv)
    const emblib::matrixf<3, 4> da_dq {
...
void task_state_estimator::run() noexcept
{
    ...
    matrixf<OBS_DIM> R(0);
    R.set_submatrix(0, 0, m_task_accel.get_noise_variance());
    R.set_submatrix(3, 3, m_task_gyro.get_noise_variance());
