...
emblib::vector3f copter::acc(const vector3f& vel, const quaternionf& rotq) const noexcept
{
    const float m = m_params.mass;
    return GRAVITY + rotq.rotate_vec((thrust() / m) * UP) - m_params.lin_drag_c * vel / m;
}

emblib::vector3f copter::ang_acc(const vector3f& vel, const quaternionf& rotq, const vector3f& ang_vel) const noexcept
{
    const matrixf<3>& I = m_params.inertia_matrix;
    const vector3f I_w = I.matmul(ang_vel);
    return (torque() - ang_vel.cross(I_w)).matdivl(I);
}

model::jacobian_s copter::jacobian(const vector3f& vel, const vector4f& rotqv, const vector3f& ang_vel) const noexcept
{
    const float m = m_params.mass;
